...
- **服务列表自动发现**：面板不再依赖写死的 `LIGHTOPS_SERVICES`。每轮采集扫描 systemd（`list-units --all` + `list-unit-files`），把"正在运行"或"开机自启"的服务纳入监控——新装的自动出现，卸载的自动消失。systemd 内部件（`systemd-*` / `dbus` / `getty@` / `dracut-*` / `user@` 等）自动剔除，不污染面板。

### 修复

- **已卸载的服务被当成故障**：unit 删除后 `systemctl is-active` 返回 not-found，旧逻辑一律按"非 active"处理，面板显示挂了并每轮刷一条 critical 告警。现在按 `LoadState` 识别为"未安装"，从面板列表移除并清理其历史告警。

### 调整

- **`LIGHTOPS_SERVICES` 语义降级为白名单**：它不再是服务列表，而是"显式关注 + 允许重启"名单（polkit 规则仍按它渲染）。名单里的名字永远出现在面板上，且只有它们带重启按钮；自动发现出来但不在名单里的服务为只读，点了也不会有权限。
...
- **服务使用方识别增强**：探针补两条信号——systemd 声明式依赖（`list-dependencies --reverse`，能抓到 Requires/Wants 关系如 nginx → php-fpm，运行时连接看不到）与**入站连接统计**（按监听端口 / Unix 路径统计入站连接，区分外部与本机）。无本地调用方的服务标注升级为"对外端口 · N 外部 / M 本机连接"。
- **服务语义标注配置**：`LIGHTOPS_SERVICE_LABELS`（如 `nginx:LightOps 面板 / Zabbix Web`）。调用方是外部浏览器 / 远程探针时自动识别给不出关系，可在服务器配置里补语义，升级时保留；标注优先级：探针识别的调用方 → 配置标注 → 对外端口与连接数。标注写在配置而非代码，换服务器仍可维护。
